...
| question | page |
|---|---|
| How do I find IT consulting tenders in Germany? | https://apify.com/lotebo-lab/ted-tender-matcher/examples/it-consulting-tenders-in-germany |
| Which construction tenders are open in Portugal? | https://apify.com/lotebo-lab/ted-tender-matcher/examples/construction-tenders-in-portugal |
| How do I find healthcare tenders in Spain? | https://apify.com/lotebo-lab/ted-tender-matcher/examples/healthcare-tenders-in-spain |
| Which EU tenders are worth more than 1 million euros? | https://apify.com/lotebo-lab/ted-tender-matcher/examples/eu-tenders-above-1-million-euros |
| How do I see only EU tenders published this week? | https://apify.com/lotebo-lab/ted-tender-matcher/examples/eu-tenders-published-this-week |
| How do I search EU tenders by CPV code for software? | https://apify.com/lotebo-lab/ted-tender-matcher/examples/search-eu-tenders-by-cpv-code-for-software |

Who runs it: companies and consultants that bid for public contracts in the European Union, and the people who prepare their pipeline. A daily or weekly tender watch that replaces a manual search with a scheduled run; narrowing a wide CPV family down to the few notices that fit, for example everything under `45000000` but only in Germany and Portugal and only above EUR 200,000; feeding a bid/no-bid pipeline where each row carries the value and the deadline; market sizing, when you want to know how many notices of a CPV family a country published in a period.
...
| fields requested | eight, from the official field list (https://docs.ted.europa.eu/ODS/latest/reuse/field-list.html) |
...
These are the fields of [`.actor/input_schema.json`](.actor/input_schema.json), with the defaults and the ranges the schema declares. Every filter is optional.

| field | type | default | notes |
|---|---|---|---|
| `cpvCodes` | array of strings | `[]` (any category) | Common Procurement Vocabulary codes. Hierarchical: `45000000` also returns everything below it, such as `45213150`. Several codes are combined with `OR` |
| `countries` | array of strings | `[]` (every country) | buyer country as a **three letter** code: `DEU`, `FRA`, `ESP`, `PRT`. A two letter code such as `DE` matches nothing and returns zero rows without an error |
| `minValueEur` | integer | `0` | drop notices whose total value is below this amount in euros. A notice published with no value is dropped whenever this is above zero |
| `keywords` | array of strings | `[]` (filter skipped) | words looked for in the notice **title**, case insensitive, `OR` between them |
| `publishedSince` | string | empty (every published notice) | date in the form `YYYY-MM-DD`, to look only at notices published on or after it. The schema rejects any other shape |
...
An example input, using the values the schema prefills:
...
An empty input is valid and returns whatever TED published, which is a lot. The sensible first run is one CPV family and one country you already follow.
...
**Stage two, applied here** (`src/matching.py`). The value filter, the keyword filter and the hierarchical CPV rule are applied locally, so a change in TED's query language can never silently widen what you receive:

- **CPV, hierarchically.** CPV is a tree, and a wanted code matches any notice code at or below it. `45000000` matches `45213150`; `45213150` does **not** match `45000000`. The rule is prefix based: the trailing zeros of the code you ask for mark the level.
- **Country**, compared upper case, exact match on the three letter code.
- **Minimum value**, a plain "greater than or equal". A notice published without a value cannot pass a minimum above zero.
- **Keywords**, plain substring match on the title, ignoring case, `OR` between them. There is no stemming: `school` does not match `schools`.
- **Dedupe** by `publication_number`, keeping the first occurrence.

Then the result is cut to `maxResults`.

## What comes out

The dataset holds two kinds of row, told apart by `rowType`: one **tender** row per matching notice, and exactly one **summary** row per run, written last. These are the fields declared in [`.actor/dataset_schema.json`](.actor/dataset_schema.json), which is the schema the Apify Console renders as the output table, and the same names the code writes.

| field | type | what it holds |
|---|---|---|
| `rowType` | string | `tender` on a notice row, `summary` on the one summary row |
...
| `buyer_name` | string or null | the contracting organisation, under the same language rule. An organisation, never a person |
...
A field TED did not fill comes back `null`, or an empty list for `cpv_codes`, rather than being missing, so a spreadsheet keeps its columns aligned. The dataset ships one ready-made view, **Matching tenders**.

The summary row carries `finishedAt`, `noticesRead` (raw notices TED answered, before any local filter), `matchedNotices`, `storedNotices`, `chargedEvents`, `chargeLimitReached`, `chargeFailures`, `criteria` (the filters of this run in one readable clause) and `message`. It exists because an empty dataset is ambiguous: "your filters matched nothing" and "the Actor broke" would look identical. The `message` tells three cases apart in words: nothing read, read but nothing matched (with which knob to widen), or matches found. Every run also writes a `SUMMARY` record in the key-value store with the same counts plus `criteriaUsed`, the criteria as a structured object.

### The eight fields it asks TED for

These are the exact names sent in the `fields` array of the request body (`ALLOWED_API_FIELDS` in `src/ted_client.py`), and the column each one becomes:

| field asked from TED | column in the dataset |
|---|---|
...
A caller cannot widen the request into another field, personal or not, even by mistake: an unknown name is dropped, and if nothing survives, the full list of eight is used.

### Example output

The rows below come from an end-to-end run of `src/main.py` recorded in our build log. In that run the `POST` to the TED API is answered from `tests/fixtures/ted_search_page1.json`, because the machine that builds this Actor has no route to TED. **The values are illustrative; the column names are the real ones.** The fixture holds 5 notices and the filters kept 2.
...
- **the request is an allow list.** The `fields` array sent to TED can only contain the eight names above. A personal field is never even asked for;
- **the output is the same allow list, applied again.** `normalize_notice` starts from an empty dictionary and writes exactly those eight keys. A field that arrives and is not one of the eight is never copied, including a field TED may add in the future;
...
The summary row itself charges nothing. Charging failures never kill a run: a charge call that does not answer within 5 seconds is logged as a warning, counted in `chargeFailures`, and the search carries on.
...
- **It does not download the tender documents**, the annexes or any attachment. It returns the link to the notice; the documents stay on ted.europa.eu.
...
- **It does not enrich the notice with company data**, registry lookups or financials.
...
- **No stemming and no plural forms in keywords.** `school` does not match `schools`. List the variants you want.
...
- **Dedupe is per run, by publication number.** A corrigendum with a different publication number is a different row. Across two runs, nothing is deduplicated.
...
- **The value is a number, as published.** Where TED publishes the amount in another currency, this Actor does not convert it.
...
- **Speed.** Pages are fetched one at a time with a pause between them, deliberately. A wide search takes minutes, not seconds.
...
- **This Actor requests no URL that you supply.** It calls one documented endpoint, `https://api.ted.europa.eu/v3/notices/search`, and nothing else. There is no crawl, so there is no `robots.txt` for this Actor to consult on your behalf.
- **It respects the limits TED publishes for reusers**: one request at a time out of the three concurrent downloads allowed, a pause of 0.25 s between pages, well under the ceiling of "700 requests in the last minute" (https://ted.europa.eu/en/simap/developers-corner-for-reusers).
- **Transient failures are retried** — HTTP 408, 425, 429, 500, 502, 503, 504 and network errors — up to 4 times with a wait that doubles from 2 s, capped at 60 s, and a numeric `Retry-After` from TED wins when it asks for longer. **HTTP 400 is deliberately not retried**, because a malformed query is our bug and must surface on the first attempt.
...
**What passes, offline.** The whole suite runs without network, with a fake transport injected in place of the HTTP client. Run from the Actor directory, on 2026-09-22:
...
| `tests/test_schemas.py` | 78/78 | the four `.actor` JSON files parse, reference each other correctly, the pay-per-event block holds exactly one event `tender-matched` matching the name the charging code uses, and every field named in a schema really exists in a row the code pushes |
...
| `.actor/` | the Actor definition, the input form, the dataset fields and views, and the run output links |
...
**Actor page on the Apify Store: https://apify.com/lotebo-lab/ted-tender-matcher**
